...
const FAQ = () => {
   ...
   const toggleActiveIndex = (index) => {
    setActiveIndices(currentActiveIndices => {
      const newActiveIndices = new Set(currentActiveIndices);
  ...
  return (
    <div className="flex flex-col md:flex-row">
      ...
      <div className="md:w-1/2 p-8 pl-48">
        ...
        {faqData.map((faq, index) => (
          <div key={index} className="mb-4">
            ...
            >
              ...
              <span>{activeIndices.has(index) ? '▲' : '▼'}</span>
            </button>
            <div className={activeIndices.has(index) ? 'block py-4' : 'hidden'}>
              <p>{faq.answer}</p>
            </div>
